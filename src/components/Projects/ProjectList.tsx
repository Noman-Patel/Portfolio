    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 pt-20 gap-5">
      {projects.map((project, idx) => (
        <div key={idx} className="flex flex-col space-y-4">
          <h1 className="text-2xl font-bold tracking-tight text-[var(--text-secondary)]">
            {project.title}
          </h1>
          ...
          <HoverEffect
              ...
              >
                <div className="space-y-5 bg-black/50 p-2 rounded">
                  <h1 className="text-2xl font-bold text-[var(--text-secondary)]">{project.title}</h1>
                  <div className="flex items-center gap-5">
                    {project.tech.map((Icon, idx) => (
                      <Icon key={idx} className="w-8 h-8 text-[var(--text-secondary)]" />
                    ))}
                  </div>
                </div>
            </HoverEffect>
